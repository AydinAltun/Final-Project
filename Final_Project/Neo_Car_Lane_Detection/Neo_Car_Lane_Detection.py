...
class genel():
    ...
    set_edilen_Fps                                              = 10
    video                                                       = ""
    otonom_surus_devrede_mi                                     = False
    gerceklesen_frame_sayisi                                    = ""
    serit_bulunamayan_frame_sayisi                              = 0
    ...
    def arac_durdu():
        ...
        genel.otonom_surus_devrede_mi = False
    ...
    def otonom_surus_kontrolu():
        if not genel.otonom_surus_devrede_mi:
            otonom_btn.config(relief=SUNKEN)
            neo_car_gui.update_idletasks()
            genel.otonom_surus_devrede_mi = True
            print("Otonom sürüş başlıyor")
        else:
            otonom_btn.config(relief=RAISED)
            neo_car_gui.update_idletasks()
            genel.otonom_surus_devrede_mi = False
            print("Otonom sürüş sonlandırılıyor")
            genel.arac_durdu()        
    ...
    def tus_basma_kontrol(event):
        if event.keysym != "5" and genel.otonom_surus_devrede_mi:
            genel.otonom_surus_devrede_mi = False
            genel.arac_durdu()
    ...
    def detect_edges(frame):
        ...
        #cv2.imshow("HSV",hsv)
        mask = cv2.inRange(hsv,genel.lower_value, genel.upper_value)
    ...
    def average_slope_intercept(frame, line_segments):
        ...
        for line_segment in line_segments:
            for x1, y1, x2, y2 in line_segment:
                if x1 == x2:
                    #şerit çizgileri düz, eğim yok, dönüş yapılmayacak düz gidilecek
                    #print("Dikey çizgiler atlanıyor (slope = infinity)")
                    continue
        ...
        if len(left_fit) > 0:
            lane_lines.append(genel.make_points(frame, left_fit_average))

        right_fit_average = np.average(right_fit, axis=0)
        if len(right_fit) > 0:
            lane_lines.append(genel.make_points(frame, right_fit_average))
    ...
    def kontrol_cevrimi():
        ...
        if ret:
            ...
            # eğer sadece manuel kontrol ile araç çalıştırılacaksa ana kamera görüntüsünü label içine koyuyoruz
            if genel.otonom_surus_devrede_mi:
                msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş   -"
                if otonom_btn.cget("relief") == "raised":
                    otonom_btn.config(relief=SUNKEN)

                genel.aracin_simdiki_gidecegi_yon = ""

                edges         = genel.detect_edges(frame)
                roi           = genel.region_of_interest(edges)
                line_segments = genel.detect_line_segments(roi)
               
                if line_segments is None:
                    genel.serit_bulunamayan_frame_sayisi += 1
                    if genel.serit_bulunamayan_frame_sayisi == 3:
                        msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş Durdu   -   Şerit bulunamadı!"
                        print("üst üste 3 frame şerit bulunamadı otonom sürüş sonlandırıldı, araç durduruldu")
                        genel.serit_bulunamayan_frame_sayisi = 0
                        genel.arac_durdu()
                else:
                    genel.serit_bulunamayan_frame_sayisi = 0
                    lane_lines       = genel.average_slope_intercept(frame,line_segments)
                    lane_lines_image = genel.display_lines(frame,lane_lines)
                    steering_angle   = genel.get_steering_angle(frame, lane_lines)
                    heading_image    = genel.display_heading_line(lane_lines_image,steering_angle)
                    
                    img = Image.fromarray(heading_image)
                    ...
                    msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş   -   Sapma : {deviation}"


                    if -5 <= deviation <= 5:
                        deviation = 0
                        error = 0
                        genel.aracin_simdiki_gidecegi_yon = "----- DÜZ ileri 1. seviye -----"
                        genel.sol_motor_hizi = 45
                        genel.sag_motor_hizi = 45
                        genel.arac_duz_ileri_gidiyor()

                    elif -30 < deviation <= -5:
                        genel.aracin_simdiki_gidecegi_yon = "SOL ileri <----------"
                        genel.sol_motor_hizi = 35
                        genel.sag_motor_hizi = 55
                        genel.arac_tam_sola_donuyor()

                    elif 5 <= deviation < 30:
                        genel.aracin_simdiki_gidecegi_yon = "SAĞ ileri <----------"
                        genel.sol_motor_hizi = 55
                        genel.sag_motor_hizi = 35
                        genel.arac_tam_saga_donuyor()

                    elif -60 < deviation <= -30:
                        genel.aracin_simdiki_gidecegi_yon = "TAM SOL 1. seviye <----------"
                        genel.sol_motor_hizi = 40
                        genel.sag_motor_hizi = 60
                        genel.arac_tam_sola_donuyor()

                    elif 30 <= deviation < 60:
                        genel.aracin_simdiki_gidecegi_yon = "TAM SAĞ 1. seviye <----------"
                        genel.sol_motor_hizi = 60
                        genel.sag_motor_hizi = 40
                        genel.arac_tam_saga_donuyor()                    

                    elif -90 < deviation <= -60:
                        genel.aracin_simdiki_gidecegi_yon = "TAM SOL 2. seviye <----------"
                        genel.sol_motor_hizi = 50
                        genel.sag_motor_hizi = 65
                        genel.arac_tam_sola_donuyor()

                    elif 60 <= deviation < 90:
                        genel.aracin_simdiki_gidecegi_yon = "TAM SAĞ 2. seviye <----------"
                        genel.sol_motor_hizi = 65
                        genel.sag_motor_hizi = 50
                        genel.arac_tam_saga_donuyor()                                        

            else:
                if otonom_btn.cget("relief") == "sunken":
                    otonom_btn.config(relief=RAISED)
        ...
        genel.kontrol_cevrimi_after_id = msg_lbl.after(50, genel.kontrol_cevrimi)
...
class acilis():
    def __init__(self):
        ...
        os.chdir('/home/pi/Desktop/Neo_Car_Lane_Detection/')
        ...
        # mavi şerit (kameranın gerçekte üst 80%'lik kısmı bozuk olduğundan değerleri akşam
        genel.lower_value = np.array([107, 84, 80], dtype = "uint8")
        genel.upper_value = np.array([127, 104, 160], dtype="uint8")
...
if __name__ == "__main__":
    ...
    otonom_img = ImageTk.PhotoImage(Image.open("otonom_44x30.png"))
    otonom_btn = Button (neo_car_gui, width=ok_btn_genislik, height=ok_btn_yukseklik, image=otonom_img, bd=1, command=genel.otonom_surus_kontrolu)
    otonom_btn.place(x=ok_btn_x - int((2 * ok_btn_genislik) - (2 * ok_btn_ara_mesafe)), y=ok_btn_y + (7 * ok_btn_ara_mesafe) + (5.5 * ok_btn_yukseklik))
    widget_list.append(otonom_btn)
