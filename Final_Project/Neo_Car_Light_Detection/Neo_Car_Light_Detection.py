...
class genel():
    ...
    def kontrol_cevrimi():
        ...
        if ret:
            ...
            # görüntüyü hsv'ye çevirince yeşil'i görüyor
            #frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

            msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Manuel Sürüş"

            kirmizi_filtre    = cv2.inRange(frame, genel.kirmizi_alt_deger, genel.kirmizi_ust_deger)
            kirmizi_filtre    = cv2.GaussianBlur(kirmizi_filtre, (3,3),2)
            kirmizi_filtre    = cv2.dilate(kirmizi_filtre, np.ones((5,5),np.uint8))
            kirmizi_filtre    = cv2.erode(kirmizi_filtre, np.ones((5,5),np.uint8))
            intRows,intColums = kirmizi_filtre.shape
            circles           = cv2.HoughCircles(kirmizi_filtre,cv2.HOUGH_GRADIENT,5,intRows/4)
            if circles is not None:
                for circle in circles[0]:
                    x,y,radius=circle
                    cv2.circle(frame,(int(x),int(y)),3,(255,255,255),-1)
                    cv2.circle(frame,(int(x),int(y)),int(radius),(0,0,255),3)
                    cv2.putText(frame,"Kirmizi", (int(x+radius),int(y-radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)

                # Kırmızı ışığı görünce araç durduruluyor
                msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş   -   Kırmızı Işık tespit edildi"
                genel.arac_durdu()


            sari_filtre       = cv2.inRange(frame, genel.sari_alt_deger, genel.sari_ust_deger)
            sari_filtre       = cv2.GaussianBlur(sari_filtre, (3,3),2)
            sari_filtre       = cv2.dilate(sari_filtre, np.ones((5,5),np.uint8))
            sari_filtre       = cv2.erode(sari_filtre, np.ones((5,5),np.uint8))
            intRows,intColums = sari_filtre.shape
            circles           = cv2.HoughCircles(sari_filtre,cv2.HOUGH_GRADIENT,5,intRows/4)
            if circles is not None:
                for circle in circles[0]:
                    x,y,radius=circle
                    cv2.circle(frame,(int(x),int(y)),3,(255,255,255),-1)
                    cv2.circle(frame,(int(x),int(y)),int(radius),(0,255,255),3)         
                    cv2.putText(frame,"Sari", (int(x+radius),int(y-radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2)

                # sarı ışığı görünce pwm 30 yapılınca motorlardan ses geliyor ancak aracı itemiyor
                msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş   -   Sarı Işık tespit edildi"
                genel.sol_motor_hizi = 30
                genel.sag_motor_hizi = 30
                genel.arac_duz_ileri_gidiyor()


            yesil_filtre      = cv2.inRange(frame, genel.yesil_alt_deger, genel.yesil_ust_deger)
            yesil_filtre      = cv2.GaussianBlur(yesil_filtre, (3,3),2)
            yesil_filtre      = cv2.dilate(yesil_filtre, np.ones((5,5),np.uint8))
            yesil_filtre      = cv2.erode(yesil_filtre, np.ones((5,5),np.uint8))
            intRows,intColums = yesil_filtre.shape
            circles           = cv2.HoughCircles(yesil_filtre, cv2.HOUGH_GRADIENT,5,intRows/4)
            if circles is not None:
                for circle in circles[0]:
                    x,y,radius=circle
                    cv2.circle(frame,(int(x),int(y)),3,(255,255,255),-1)
                    cv2.circle(frame,(int(x),int(y)),int(radius),(0,255,0),3)
                    cv2.putText(frame,"Yesil", (int(x+radius),int(y-radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)

                # yeşil ışığı görünce pwm 80 yapılınca motorlar aracı itebiliyor, araç hareket ediyor
                msg_lbl["text"] = f"FPS : {genel.gerceklesen_frame_sayisi}/{genel.set_edilen_Fps}   -   Otonom Sürüş   -   Yeşil Işık tespit edildi"
                genel.sol_motor_hizi = 80
                genel.sag_motor_hizi = 80
                genel.arac_duz_ileri_gidiyor()
...
class acilis():
    def __init__(self):
        ...
        os.chdir('/home/pi/Desktop/Neo_Car_Light_Detection/')
        ...
        genel.kirmizi_alt_deger = np.array([0,0,190])
        genel.kirmizi_ust_deger = np.array([100,100,255])
        #sari renk değerlikleri
        genel.sari_alt_deger    = np.array([190,0,0])
        genel.sari_ust_deger    = np.array([255,100,100])
        #yeşil renk değerlikleri
        genel.yesil_alt_deger   = np.array([60,236,168])
        genel.yesil_ust_deger   = np.array([80,256,248])
